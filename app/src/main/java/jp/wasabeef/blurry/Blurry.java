...
public class Blurry {
  ...
  public static class Composer {
    ...
    public void onto(final ViewGroup target) {
      ...
      if (async) {
        AsyncBlurTask task = new AsyncBlurTask(target, factor, new AsyncBlurTask.Callback() {
          @Override
          public void done(Bitmap bitmap) {
            final BitmapDrawable drawable =
                    new BitmapDrawable(target.getResources(), Blur.of(context, bitmap, factor));
            addBlurredView(target, drawable);
          }
        });
        task.execute();
      } else {
    ...
    private void addBlurredView(ViewGroup target, Drawable drawable) {
  ...
  public static class BitmapComposer {
    ...
    public void into(final ImageView target) {
      ...
      if (async) {
        AsyncBlurTask task = new AsyncBlurTask(target.getContext(), bitmap, factor, new AsyncBlurTask.Callback() {
          @Override
          public void done(Bitmap bitmap) {
  ...
  public static class ImageComposer {
    ...
    public void into(final ImageView target) {
      ...
      if (async) {
        AsyncBlurTask task = new AsyncBlurTask(capture, factor, new AsyncBlurTask.Callback() {
          @Override
          public void done(Bitmap bitmap) { //368x624
            //FIXME 存在density导致bitmap大小异常的问题  BitmapDrawable的targetDensity计算错误。
            BitmapDrawable drawable = new BitmapDrawable(context.getResources(), bitmap);
            target.setImageDrawable(drawable); //565x958  1.5353比例
          }
        });
        task.execute();
      } else {
    ...
    public Bitmap get() {
      if (async) throw new IllegalArgumentException("Use getAsync() instead of get().");
      factor.width = capture.getMeasuredWidth();
      factor.height = capture.getMeasuredHeight();
      return Blur.of(capture, factor);
    }

    public void getAsync(AsyncBlurTask.Callback callback) {
      factor.width = capture.getMeasuredWidth();
      factor.height = capture.getMeasuredHeight();
      new AsyncBlurTask(capture, factor, callback).execute();
    }
  }
}
